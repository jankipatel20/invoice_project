...
from utils.pipeline import process_invoice, LANG_CONFIG
...
def process():
    # Accept JSON (base64) or multipart form
    if request.is_json:
        data      = request.get_json()
        b64_data  = data.get("image_b64", "")
        src_lang  = data.get("src_lang", "french").lower()
        ...
        src_lang = request.form.get("src_lang", "french").lower()
...
def _make_preview_b64(img_array: np.ndarray) -> str:
    """Convert OpenCV array to base64 JPEG string for the UI."""
    try:
